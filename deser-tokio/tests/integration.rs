...
#[test]
fn test_values_in_small_chunks() {
    run(async {
...
#[test]
fn test_read_is_cancellation_safe() {
    run(async {
        let (mut client, server) = duplex(64);
        let mut reader = Reader::new(server, LINES.decoder());
        // half a value arrives, then the read is cancelled
        client.write_all(b"{\"id\": 1, ").await.unwrap();
        let read = tokio::time::timeout(Duration::from_millis(10), reader.read::<Message>());
        assert!(read.await.is_err(), "value is incomplete");
        client
...
#[test]
fn test_codec_values_at_the_end() {
    run(async {
