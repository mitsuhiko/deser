...
use deser::{Deserialize as DeserDeserialize, Serialize as DeserSerialize};
...
#[bench]
fn bench_deserialize_deser_json(b: &mut Bencher) {
    let j = input_json();
    b.iter(|| {
...
#[bench]
fn bench_serialize_deser_json(b: &mut Bencher) {
    let s = input_struct();
    b.iter(|| {
...
#[derive(
    Serialize, DeserSerialize, Deserialize, DeserDeserialize, MiniSerialize, MiniDeserialize,
)]
struct Twitter {
    statuses: Vec<Status>,
