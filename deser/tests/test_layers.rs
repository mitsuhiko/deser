...
use deser::{Atom, Descriptor, Deserialize, Error, ErrorKind, Event, Serialize, State};
...
fn test_borrowed_events_pass_through_layers() {
    ...
    let log = Log::default();
    let value: Vec<&str> = Events(vec![Event::SeqStart, input.as_str().into(), Event::SeqEnd])
...
fn test_limits() {
    ...
    assert_eq!(
        check(
            ...
            vec![Event::SeqStart, "abcd".into(), Event::SeqEnd]
        ),
...
fn test_error_context() {
    ...
    let err = emit_all(
        &mut driver,
        vec![Event::SeqStart, Event::SeqStart, 1u64.into(), true.into()],
    )
    .unwrap_err();
    ...
    let err = emit_all(
        &mut driver,
        vec![Event::SeqStart, Event::SeqStart, "x".into()],
    )
    .unwrap_err();
    ...
    let err = emit_all(&mut driver, vec![Event::SeqStart, "x".into()]).unwrap_err();
...
#[test]
fn test_format() {
    let value: Vec<u32> = Events(vec![Event::SeqStart, 1u64.into(), Event::SeqEnd])
        .deserialize()
        .unwrap();
    assert_eq!(value, [1]);
...
fn test_ser_layers_compose_with_delayed_keys() {
    ...
    assert_eq!(
        events,
        [Event::MapStart, "A".into(), 1u64.into(), Event::MapEnd]
    );
}
